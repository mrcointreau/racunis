...
const main = async () => {
  // Create a MySqlQueue instance with the specified connection details
  ...
  const jobs = [
    queue.add({ type: 'EmailProcessing', details: { recipient: 'admin@example.com', content: 'Greetings!' } }, 6),
    queue.add({ type: 'DataMigration', details: { source: 'System B', destination: 'System C' } }, 2),
    queue.add({ type: 'ReportGeneration', details: { reportId: 'R456', requestedBy: 'user' } }, 5),
  ]
  ...
  // Create a MySqlWorker to process the jobs
  const worker = MySqlWorker.create(queue, processJob, { autostart: false })
